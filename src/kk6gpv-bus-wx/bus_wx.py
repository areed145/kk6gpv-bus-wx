...
async def wx_on_message(ws):
    while True:
        ...
        if message["type"] == "obs_air":
            ...
            msg["dewpoint_f"] = str(
                np.round(
                    (message["obs"][0][2] - (100 - message["obs"][0][3]) / 5)
                    * (9 / 5)
                    + 32,
                    2,
                )
            )
            msg["relative_humidity"] = str(np.round(message["obs"][0][3], 2))
            msg["pressure_in"] = str(
                np.round(message["obs"][0][1] * 0.029693, 3)
            )
            msg["pressure_trend"] = str(message["summary"]["pressure_trend"])
            msg["strike_count_3h"] = str(message["summary"]["strike_count_3h"])
        ...
        if message["type"] == "obs_sky":
            ...
            msg["wind_mph"] = str(np.round(message["obs"][0][5] * 1.94384, 2))
